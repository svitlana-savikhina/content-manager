...
class TestPostFunctions(unittest.TestCase):
    ...
    def test_get_post_by_id(self):
        post_id = 1
        mock_post = models.Post(id=post_id, title="Test Post", content="Test content", user_id=1)
    ...
    def test_update_post_by_id(self):
        ...
        mock_post = models.Post(id=post_id, title="Test Post", content="Test content", user_id=user_id)
        self.mock_db_session.query().filter().first.return_value = mock_post

        check_profanity = MagicMock(return_value=(False, ""))

        with patch('posts.crud.check_profanity', check_profanity):
            updated_post = update_post_by_id(self.mock_db_session, post_id, post_data, user_id)
    ...
    def test_create_comment(self):
        ...
        with patch('posts.crud.check_profanity', check_profanity):
            created_comment = create_comment(self.mock_db_session, comment_data, user_id, post_id)
    ...
    def test_create_comment_with_profane_content(self):
        ...
        with patch('posts.crud.check_profanity', check_profanity):
            with self.assertRaises(HTTPException) as cm:
                create_comment(self.mock_db_session, comment_data, comment_data.user_id, comment_data.post_id)
    ...
    def test_get_comments_for_post(self):
        post_id = 1
        mock_comments = [
            models.Comment(id=1, content="Comment 1", user_id=1, post_id=post_id, blocked=False),
            models.Comment(id=2, content="Comment 2", user_id=2, post_id=post_id, blocked=False),
        ]
        self.mock_db_session.query().filter().offset().limit().all.return_value = mock_comments
    ...
    def test_update_comment(self):
        comment_id = 1
        comment_data = schemas.CommentUpdate(content="Updated comment content", updated_at=datetime.now())

        mock_comment = models.Comment(id=comment_id, content="Test comment", user_id=1, post_id=1, blocked=False)
        self.mock_db_session.query().filter().first.return_value = mock_comment

        check_profanity = MagicMock(return_value=(False, ""))

        with patch('posts.crud.check_profanity', check_profanity):
            updated_comment = update_comment(self.mock_db_session, comment_id, comment_data)
    ...
    def test_delete_comment_by_id_and_post_id(self):
        comment_id = 1
        post_id = 1
        mock_comment = models.Comment(id=comment_id, content="Test comment", user_id=1, post_id=post_id, blocked=False)
        self.mock_db_session.query().filter().first.return_value = mock_comment

        result = delete_comment_by_id_and_post_id(self.mock_db_session, comment_id, post_id)
    ...
    def test_get_comments_data(self):
        ...
        self.assertEqual(len(result), 2)  # Должно быть 2 даты в результате
